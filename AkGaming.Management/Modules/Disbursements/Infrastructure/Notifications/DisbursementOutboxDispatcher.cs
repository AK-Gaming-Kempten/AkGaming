...
public sealed class DisbursementOutboxDispatcher(
    IServiceScopeFactory scopeFactory,
    IHttpClientFactory httpClientFactory,
    IOptions<DisbursementNotificationOptions> options,
    ILogger<DisbursementOutboxDispatcher> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            logger.LogWarning("The disbursement notification outbox dispatcher is disabled because Notifications:Endpoint is not configured.");
            return;
        }
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(exception, "The disbursement notification outbox dispatcher failed while polling.");
    ...
    private async Task<bool> DispatchNextAsync(CancellationToken cancellationToken)
    {
        ...
        var dbContext = scope.ServiceProvider.GetRequiredService<DisbursementsDbContext>();
        ...
        try
        {
            var tokenProvider = scope.ServiceProvider.GetRequiredService<NotificationAccessTokenProvider>();
            ...
            if (!response.IsSuccessStatusCode)
                throw new HttpRequestException($"Discord notification API returned {(int)response.StatusCode}: {responseBody}");
        ...
        catch (Exception exception)
        {
            ...
            message.NextAttemptAtUtc = DateTimeOffset.UtcNow.AddSeconds(Math.Min(300, Math.Pow(2, Math.Min(10, message.AttemptCount))));
            logger.LogWarning(exception, "Outbox event {EventId} could not be submitted on attempt {AttemptCount}.", message.EventId, message.AttemptCount);
        }
