...
internal static class MembershipDueReminderEmailComposer {
    ...
    private const string MembershipFeeUrl = "https://akgaming.de/mitgliedschaft/mitgliedsbeitrag";
    private const string SatzungUrl = "https://akgaming.de/Vereinssatzung-AK-Gaming-e.V..pdf";
    private const string BeitragsordnungUrl = "https://akgaming.de/Beitragsordnung-AK-Gaming-e.V..pdf";
    private const string LogoUrl = "https://akgaming.de/assets/akgaming_logo.png";
        ...
        MembershipDue due)
    {
        ...
        var subject = $"AK Gaming e.V. | Offener Mitgliedsbeitrag {paymentPeriod.Name}";
        ...
        string remainingAmountText)
    {
        ...
        text.AppendLine();
        text.AppendLine("AK Gaming e.V.");
        text.AppendLine("IBAN: DE59 7336 9920 0000 8872 85");
        text.AppendLine("BIC: GENODEF1SFO");
        ...
        text.AppendLine($"Stelle einen formlosen Antrag per Mail an {VorstandEmail}, um Fördermitglied zu werden, und überweise den verringerten Beitrag für Fördermitglieder (frei wählbar zwischen 5 € und 15 €) an das oben genannte Konto.");
        text.AppendLine($"Alle Informationen zur Fördermitgliedschaft findest du unter {MembershipFeeUrl}.");
        text.AppendLine();
        text.AppendLine("3. BEITRAGSERMÄSSIGUNG BZW. -BEFREIUNG BEANTRAGEN");
        text.AppendLine($"Solltest du dich aktuell in einer finanziell schwierigen Lage befinden, kannst du einen formlosen Antrag per Mail an {VorstandEmail} stellen, um eine Beitragsermäßigung oder -befreiung zu erhalten.");
        ...
        text.AppendLine($"Alle Informationen zur Beitragsermäßigung und -befreiung findest du unter {MembershipFeeUrl}.");
        text.AppendLine();
        text.AppendLine("4. AUS DEM AK GAMING E.V. AUSTRETEN");
        text.AppendLine($"Auch wenn wir dich nur ungern als Mitglied verlieren, ist eine formlose Austrittserklärung per Mail an {VorstandEmail} für alle Beteiligten einfacher als ein Suspendierungsverfahren.");
        text.AppendLine();
        ...
        text.AppendLine("Vorstand AK Gaming e.V.");
        text.AppendLine();
        ...
        text.AppendLine($"Mitgliedsbeitrag: {MembershipFeeUrl}");
        text.AppendLine($"Vereinssatzung: {SatzungUrl}");
        text.AppendLine($"Beitragsordnung: {BeitragsordnungUrl}");
        ...
        string remainingAmountText)
    {
        ...
        html.Append($"<img src=\"{LogoUrl}\" alt=\"AK Gaming Logo\" width=\"56\" height=\"56\" style=\"display:block;width:56px;height:56px;border-radius:14px;background:rgba(255,255,255,0.12);padding:6px;\" />");
        html.Append("<div>");
        html.Append("<p style=\"margin:0 0 6px;font-size:12px;font-weight:700;letter-spacing:0.14em;text-transform:uppercase;color:#c0e1c7;\">AK Gaming e.V.</p>");
        ...
        html.Append("</table>");
        html.Append("</div>");
        html.Append($"<div style=\"margin-top:18px;\"><a href=\"{MembershipFeeUrl}\" style=\"display:inline-block;padding:11px 18px;border-radius:999px;background:#286c3f;color:#ffffff;text-decoration:none;font-weight:700;\">Zur Beitragsseite</a></div>");
        html.Append("</div>");
        html.Append("</div>");
        ...
        AppendOptionCard(
            ...
            "<div><strong>AK Gaming e.V.</strong></div>" +
            "<div>IBAN: <strong>DE59 7336 9920 0000 8872 85</strong></div>" +
            "<div>BIC: <strong>GENODEF1SFO</strong></div>" +
        ...
        AppendOptionCard(
            ...
            $"Stelle einen formlosen Antrag per Mail an <a href=\"mailto:{VorstandEmail}\" style=\"color:#286c3f;\">{VorstandEmail}</a>, um Fördermitglied zu werden, und überweise den verringerten Beitrag für Fördermitglieder (frei wählbar zwischen 5 € und 15 €) an das oben genannte Konto. " +
            $"Alle Informationen findest du unter <a href=\"{MembershipFeeUrl}\" style=\"color:#286c3f;\">akgaming.de/mitgliedschaft/mitgliedsbeitrag</a>.");

        AppendOptionCard(
            html,
            "Beitragsermäßigung bzw. -befreiung beantragen",
            $"Solltest du dich aktuell in einer finanziell schwierigen Lage befinden, kannst du einen formlosen Antrag per Mail an <a href=\"mailto:{VorstandEmail}\" style=\"color:#286c3f;\">{VorstandEmail}</a> stellen, um eine Beitragsermäßigung oder -befreiung zu erhalten. " +
            $"Überweise nach Genehmigung des Antrags den für dich festgelegten verringerten Beitrag an das oben genannte Konto. Weitere Informationen findest du unter <a href=\"{MembershipFeeUrl}\" style=\"color:#286c3f;\">akgaming.de/mitgliedschaft/mitgliedsbeitrag</a>.");

        AppendOptionCard(
            html,
            "Aus dem AK Gaming e.V. austreten",
            $"Auch wenn wir dich nur ungern als Mitglied verlieren, ist eine formlose Austrittserklärung per Mail an <a href=\"mailto:{VorstandEmail}\" style=\"color:#286c3f;\">{VorstandEmail}</a> für alle Beteiligten einfacher als ein Suspendierungsverfahren.");
        ...
        html.Append("<p style=\"margin:20px 0 0;\">Liebe Grüße<br/><strong>Vorstand AK Gaming e.V.</strong></p>");
        html.Append("</div>");
        ...
        html.Append($"<a href=\"{MembershipFeeUrl}\" style=\"color:#286c3f;\">Mitgliedsbeitrag</a> · ");
        html.Append($"<a href=\"{SatzungUrl}\" style=\"color:#286c3f;\">Vereinssatzung</a> · ");
        html.Append($"<a href=\"{BeitragsordnungUrl}\" style=\"color:#286c3f;\">Beitragsordnung</a>");
        html.Append("</p>");
        html.Append("</div>");
