...
public partial class InvoiceLineItemCollectionPresetsPanel : ComponentBase
{
    ...
    private List<InvoiceLineItemCollectionPresetDto>? _presets; private InvoiceLineItemCollectionPresetDto _editing = New(); private bool _showCreate; private bool _isBusy; private string? _error; private string? _status;
    ...
    private void Select(InvoiceLineItemCollectionPresetDto p) { _showCreate = false; _editing = Clone(p); Clear(); }
    ...
    private async Task DeleteAsync() { _isBusy = true; Clear(); var r = await Api.DeleteLineItemCollectionPresetAsync(_editing.Id); _isBusy = false; if (!r.IsSuccess) { _error = r.Error; return; } _editing = New(); _status = "Collection deleted."; await LoadAsync(); }
