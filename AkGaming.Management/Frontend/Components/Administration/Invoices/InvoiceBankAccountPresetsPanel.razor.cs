...
public partial class InvoiceBankAccountPresetsPanel : ComponentBase
{
    ...
    private void ToggleCreate() { _showCreate = !_showCreate; _editing = new(); ClearMessages(); }
    private void Select(InvoiceBankAccountPresetDto preset) { _showCreate = false; _editing = Clone(preset); ClearMessages(); }
    ...
    private async Task DeleteAsync() { _isBusy = true; ClearMessages(); var result = await Api.DeleteBankAccountPresetAsync(_editing.Id); _isBusy = false; if (!result.IsSuccess) { _error = result.Error; return; } _editing = new(); _status = "Preset deleted."; await LoadAsync(); }
