...
public partial class InvoiceLineItemPresetsPanel : ComponentBase
{
    ...
    private List<InvoiceLineItemPresetDto>? _presets; private InvoiceLineItemPresetDto _editing = new() { LineItem = new() { Quantity = 1m } }; private bool _showCreate; private bool _isBusy; private bool _isMobileDetailOpen; private string? _error; private string? _status;
    ...
    private void Select(InvoiceLineItemPresetDto p) { _showCreate = false; _editing = Clone(p); _isMobileDetailOpen = true; Clear(); }
    ...
    private async Task DeleteAsync() { _isBusy = true; Clear(); var r = await Api.DeleteLineItemPresetAsync(_editing.Id); _isBusy = false; if (!r.IsSuccess) { _error = r.Error; return; } _editing = New(); _isMobileDetailOpen = false; _status = "Preset deleted."; await LoadAsync(); }
