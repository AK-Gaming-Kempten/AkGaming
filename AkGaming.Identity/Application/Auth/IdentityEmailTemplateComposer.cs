...
internal sealed record IdentityEmailAction(string Label, string Url);
internal sealed record IdentityEmailSummaryItem(string Label, string Value);

internal static class IdentityEmailTemplateComposer
{
    public static string ComposeHtml(
        string eyebrow,
        string title,
        string introHtml,
        IEnumerable<IdentityEmailSummaryItem>? summaryItems,
        IEnumerable<IdentityEmailAction>? actions,
        string bodyHtml,
        string signOffHtml,
        ...
        bool includeAutomatedNotice = true)
    {
        ...
        if (actionList.Count > 0)
        {
            ...
            {
                html.Append($"<a href=\"{H(action.Url)}\" style=\"display:inline-block;padding:11px 18px;border-radius:999px;background:#286c3f;color:#ffffff;text-decoration:none;font-weight:700;\">{H(action.Label)}</a>");
            }
        ...
        {
            html.Append("<p style=\"margin:0 0 8px;\">This email was generated automatically by AK Gaming Identity.</p>");
        }
    ...
    {
        var html = new StringBuilder();
        html.Append("<div style=\"margin-top:12px;padding:18px;border-radius:14px;background:#fbfdfb;border:1px solid #d6e8da;\">");
